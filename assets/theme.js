...
`;
  ...
  var ScrollableContent = class extends CustomHTMLElement {
    ...
    _checkScrollability() {
      // accessibility 360 issue #101, for multi-column section we need to use <ul></ul>, so look nested for ul tag below
  ...
  var GalleryList = class extends HTMLElement {
    connectedCallback() {
      ...
      this.scrollBarWrapperElement = this.querySelector(".gallery__progress-bar-wrapper");
    ...
    _updateProgressBar(event) {
      ...
      var _b;
      (_b = this.scrollBarWrapperElement) == null ? void 0 : _b.setAttribute("aria-valuenow", `${event.detail.progress}`);
    }
  };
  ...
  var ProductVariants = class extends CustomHTMLElement {
    ...
    _updateDisableSelectors() {
      ...
      const applyClassToSelector = (selector, valueIndex, available, hasAtLeastOneCombination) => {
        ...
        console.log('selector.querySelector(cssSelector)', selector.querySelector(cssSelector));

        if ( selector.querySelector(cssSelector)) {
      ...
      if (this.optionSelectors && this.optionSelectors[0]) {
        this.product["options"][0]["values"].forEach((value, valueIndex) => {
          ...
          if (this.optionSelectors[1] && selectedVariant) {
        ...
        case "open":
          if (this.open) {
            ...
            if (!MediaFeatures.prefersReducedMotion()) {
              ...
              // theme customization to set focus - #79 accessible360
              // set focus to checkout button, if not found, set to close button
              const closeButton = this.querySelector(".drawer__close-button");
    ...
    _onShow(event) {
      ...
      if (!this.isInsideDrawer) {
        ...
        <button class="cart-notification__close tap-area hidden-phone" data-action="close">
          ...
          <svg role="img" aria-labelledby="closeModal" focusable="false" width="14" height="14" class="icon icon--close icon--inline" viewBox="0 0 14 14">
            <title id="closeModal">Close Modal</title>
 ...
 // js/custom-element/section/image-with-text-overlay/image-with-text-overlay.js
 var ModalRollup = class extends CustomHTMLElement {
  connectedCallback() {
    // if (this.hasAttribute("parallax") && !MediaFeatures.prefersReducedMotion()) {
    //   this._hasPendingRaF = false;
    //   this._onScrollListener = this._onScroll.bind(this);
  ...
  async _setupVisibility() {
    await this.untilVisible();
    const image = this.querySelector(".image-overlay__image"), headings = await resolveAsyncIterator(this.querySelectorAll("split-lines, .button-group, .button-wrapper, .heading-animate")), prefersReducedMotion = MediaFeatures.prefersReducedMotion();
    await imageLoaded(image);
    const innerEffect = [
      new CustomKeyframeEffect(image, { opacity: [0, 1], transform: [`scale(${prefersReducedMotion ? 1 : 1.1})`, "scale(1)"] }, { duration: 500, easing: "cubic-bezier(0.65, 0, 0.35, 1)" }),
      new ParallelEffect(headings.map((item, index) => {
      ...
      new CustomKeyframeEffect(this.querySelector(".heading"), { opacity: [0, 1] }, { duration: 300 })
    ];
    const animation = prefersReducedMotion ? new CustomAnimation(new ParallelEffect(innerEffect)) : new CustomAnimation(new SequenceEffect(innerEffect));
    this._hasSectionReloaded ? animation.finish() : animation.play();
  }
...
window.customElements.define("modal-roll-up", ModalRollup);
